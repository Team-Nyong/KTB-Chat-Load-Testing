...
public class ChatMessageHandler {
    ...
    public void handleChatMessage(SocketIOClient client, ChatMessageRequest data) {
        ...
        try {
            ...
            if ("file".equals(messageType)) {
                ...
                Map<String, Object> fileRawData = data.getFileData();
                if (fileRawData == null || fileRawData.get("_id") == null) {
                    throw new IllegalArgumentException("파일 데이터가 올바르지 않습니다.");
                }

                String fileId = (String) fileRawData.get("_id");
                fileData = fileRepository.findById(fileId).orElse(null);

                if (fileData == null || !fileData.getUser().equals(socketUser.id())) {
                    throw new IllegalStateException("파일을 찾을 수 없거나 접근 권한이 없습니다.");
                }

                message = createMessageObject(roomId, socketUser.id(), messageContent, MessageType.file);
                message.setFileId(fileId);

                Map<String, Object> metadata = new HashMap<>();
                metadata.put("fileType", fileData.getMimetype());
                metadata.put("fileSize", fileData.getSize());
                metadata.put("originalName", fileData.getOriginalname());
                message.setMetadata(metadata);
            } else if ("text".equals(messageType)) {
                if (!messageContent.isEmpty()) {
                    message = createMessageObject(roomId, socketUser.id(), messageContent, MessageType.text);
                }
            } else {
            ...
            MessageResponse response = messageResponseMapper.mapToMessageResponse(savedMessage, sender, fileData);
    ...
    private Message createMessageObject(String roomId, String userId, MessageContent messageContent, MessageType type) {
